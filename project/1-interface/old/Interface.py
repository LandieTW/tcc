...
This application automatize the DVC 1st extremity analysis
for the installations stages, in conformity with the technical specification
...
import json
from st_aggrid import AgGrid  # grid / table list
from st_aggrid import GridUpdateMode  # dict update lib
from st_aggrid import GridOptionsBuilder  # grid / table configure lib
...
# IMAGES
image1_path = Image.open('image/Screenshot_1.jpg')
image2_path = Image.open('image/Screenshot_2.jpg')
image3_path = Image.open('image/Screenshot_3.jpg')
image4_path = Image.open('image/Screenshot_4.jpg')


def show_table(dict_data, config):
    """
...
def creating_table(dict_data, column_1, column_2):
    """
...
def st_image_input(path, caption):
    return st.image(
                path,
                caption=caption,
...
grid_options1 = creating_table(
    ...
    "Bend Moment [N.m]")
data2 = pd.DataFrame({
    'Distance from flange [m]': [''] * 50,
    'Flange Height to the seabed [mm]': [''] * 50
})
...
with tab0:
    ...
    with col2:
        ...
        st.text(
            "STAGES OF DYNAMIC SIMULATION"
            "\n"
            "Stage 1 (2,15s)"
            "\n"
            "Heave up of 2,5m at the suspended end of \n"
            "the pipe through winch "
            "retraction"
            "\n"
            "Stage 2 (50s)"
            "\n"
            "Stabilization time of forces at the flange"
            "\n"
            "Stage 3 (100s)"
            "\n"
            "Deployment of 10m of flexible pipe, paying \n"
            "out winch at a speed "
            "of 0,1m/s"
        )
    with col3:
        st.write("")
...
with tab2:
    ...
    if type_bend_restrictor == "Polymer":
        with col3:
            zr_ident_bend_restrictor = st.text_input("28. RZ Structure Number")
            zr_version_bend_restrictor = st.text_input(
                "29. RZ Document Version")
            zr_wt_air_bend_restrictor = st_number_input(
                "30. RZ Wt in Air [kg]")
            st.markdown(
                '<div class="tooltip">Wt in S/W Tooltip'
                '<span class="tooltip_text">'
            ...
            zr_wt_sw_bend_restrictor = st_number_input("31. RZ Wt in S/W [kg]")
        with col4:
            zr_length_bend_restrictor = st_number_input(
                "32. RZ Total Length [mm]")
            zr_od_bend_restrictor = st_number_input(
                "33. RZ Outside Diameter [mm]")
            zr_id_bend_restrictor = st_number_input(
                "34. RZ Internal Diameter [mm]")
            zr_contact_diameter_bend_restrictor = st_number_input(
...
with (tab6):
    ...
    with col3:
        st.write("CVD 1ª - MCV no Hub com linha suspensa (Caso 3i.A)")
    ...
    with col4:
        st.write("CVD 1ª - MCV no Hub com linha suspensa (Caso 3i.B)")
    ...
    with col5:
        st.write("CVD 1ª - MCV no Hub (Caso 3ii)")
    ...
    if st.button("Data Review"):
        with st.spinner("Processando... Por favor, aguarde!"):
            ...
            with col7_1:
                ...
                dict_linha = {
                    'ID da Linha': ident_line,
                    'Revisao': version_line,
                    'Peso vazio no ar [kg/m]': wt_air_line,
                    'Peso cheio no ar [kg/m]': sw_filled_air_line,
                    'Peso vazio na agua [kg/m]': air_filled_sw_line,
                    'Peso cheio na agua [kg/m]': sw_filled_sw_line,
                    'LDA [m]': water_depth,
                    'Diametro de contato [mm]': contact_diameter_line,
                    'Diametro nominal ["]': nominal_diameter_line,
                    'MBR storage [m]': mbr_storage_line,
                    'MBR installation [m]': mbr_installation_line,
                    'Bending Stiffness [kN.m²]': bending_stiffness_line,
                    'Torsional Stiffness [kN.m²]': torsional_stiffness_line,
                    'Axial Stiffness [kN]': axial_stiffness_line,
                    'Rel. Elong. (%) for 100kN': rel_elong_line
                }
                dict_linha_dataframe = st.dataframe(dict_linha)
            ...
            with col7_2:
                ...
                dict_vertebra = {
                    'ID da Vertebra': ident_bend_restrictor,
                    'Revisao': version_bend_restrictor,
                    'Material da vertebra': type_bend_restrictor,
                    'Comprimento [mm]': length_bend_restrictor,
                    'Peso vazio no ar [kg]': wt_air_bend_restrictor,
                    'Peso vazio na agua [kg]': wt_sw_bend_restrictor,
                    'OD [mm]': od_bend_restrictor,
                    'ID [mm]': id_bend_restrictor,
                    'Diametro de contato [mm]':
                        contact_diameter_bend_restrictor,
                    'MBR travamento [m]': locking_mbr_bend_restrictor,
                    'MF Admissivel [kN.m]': bend_moment_bend_restrictor,
                    'Cortante Adm. [kN]': shear_stress_bend_restrictor
                }
                dict_bend_restrictor_dataframe = st.dataframe(dict_vertebra)
                if type_bend_restrictor == "Polymer":
                    st.write("Rigid Zone's Bend Restrictor's Data")
                    dict_zr_vertebra = {
                        'zr-Identificacao': zr_ident_bend_restrictor,
                        'zr-Revisao': zr_version_bend_restrictor,
                        'zr-Comprimento [mm]': zr_length_bend_restrictor,
                        'zr-Peso vazio no ar [kg]': zr_wt_air_bend_restrictor,
                        'zr-Peso vazio na agua [kg]': zr_wt_sw_bend_restrictor,
                        'zr-OD [mm]': zr_od_bend_restrictor,
                        'zr_ID [mm]': zr_id_bend_restrictor,
                        'zr-Contact diameter [mm]':
                            zr_contact_diameter_bend_restrictor
                    }
                    dict_rz_dataframe = st.dataframe(dict_zr_vertebra)
                    dict_vertebra.update(dict_zr_vertebra)
            with col7_3:
                st.write("End Fitting's Data")
                dict_conector = {
                    'ID do Conector': ident_end_fitting,
                    'Revisao': version_end_fitting,
                    'Peso vazio no ar [kg]': wt_air_end_fitting,
                    'Peso vazio na agua [kg]': wt_sw_end_fitting,
                    'Comprimento [mm]': length_end_fitting,
                    'OD [mm]': od_end_fitting,
                    'ID [mm]': id_end_fitting,
                    'Diametro de contato [mm]': contact_diameter_end_fitting
                }
                dict_end_fitting_dataframe = st.dataframe(dict_conector)
                st.write("Flange Adapter's Data")
                dict_flange = {
                    'ID do Adaptador': ident_flange,
                    'Revisao': version_flange,
                    'Peso vazio no ar [kg]': wt_air_flange,
                    'Peso vazio na agua [kg]': wt_sw_flange,
                    'Comprimento [mm]': length_flange,
                    'OD [mm]': od_flange,
                    'ID [mm]': id_flange,
                    'Diametro de contato [mm]': contact_diameter_flange
                }
                dict_flange_dataframe = st.dataframe(dict_flange)
            with col7_4:
                st.write("VMC's Data")
                dict_mcv = {
                    'Equipamento submarino': subsea_equipment,
                    'Revisao': version_vcm,
                    'Fornecedor': supplier_vcm,
                    'ID do Projeto': drawing_vcm,
                    'Tipo de Equipamento': subsea_equipment_type,
                    'Peso submerso [kgf]': wt_sw_vcm,
                    'Ângulo de inclinaçao [°]': declination,
                    'A [mm]': a_vcm,
                    'B [mm]': b_vcm,
                    'C [mm]': c_vcm,
                    'D [mm]': d_vcm,
                ...
                dict_vcm_dataframe = st.dataframe(dict_mcv)
