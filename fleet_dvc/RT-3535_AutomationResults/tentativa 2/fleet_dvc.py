...
rl_config = [
    [8.5, 11.5],         # Positions [m]
    [200, 100]      # Submerged mass [kg]
    ]

'Structural limits for each analysis case'
structural_limits = {
    "2": [9.72, -16.43, 52.46],      # Normal, Shear force, Bend moment
    "3i": [10.08, -11.72, 76.65],
    "3ii": [13.12, -24.63, -118.42]
    }

'Buoys positions'
NEAR_VCM_POSITIONS = [8.5, 11.5, 14.5]
FAR_VCM_POSITIONS = [9.5, 13.5, 17.5]
...
def error_treatment(
    ...
    if N_RUN_ERROR == 0:
        ...
        if line.CumulativeLength[-1] < abs(env.WaterDepth):
            model["A/R"].StageValue[0] = abs(env.WaterDepth) - line.CumulativeLength[-1] - 40
            line.EndAZ = - model["A/R"].StageValue[0]
